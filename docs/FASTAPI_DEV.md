...
- 관리자 전용 도구로, 단일 또는 소수 관리자 환경을 전제 진행
- 브라우저에서 직접 사용하는 UI이므로 단순한 인증 방식 필요
- 인증 정보는 로그인 시 1회만 사용하고 이후는 세션으로 유지

→ 아이디/비밀번호 기반 로그인 후 세션 쿠키로 인증 유지
...
- 외부 사용자 인증 시스템이 아님
- 토큰 갱신, 서명 검증, 만료 관리 구조 불필요
- 단일 관리자 환경에서는 오버엔지니어링

→ 서버 메모리 기반 세션 관리 구조 선택
...
- 인증 없는 요청은 즉시 차단 (401)
- 로그인 성공 시에만 세션 쿠키를 발급
- 쿠키에는 인증 정보가 아닌 랜덤 세션 토큰만 저장
- 관리자 세션은 HttpOnly 쿠키로 관리
- 세션은 서버 메모리에서 관리되며, 만료 시 자동 무효화
- 브라우저는 로그인 완료 전까지 보호 API를 호 금지
