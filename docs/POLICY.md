...
### 1.1 판단 로직의 OS 독립성 (Deterministic)
- OS 정보는 수집 단계에서 한 번만 판별하여 프로세스 데이터(dict)에 명시적으로 포함
- 분석 단계는 입력된 프로세스 데이터(dict)만을 기반으로 판단
- 동일한 입력은 항상 동일한 분석 결과 보장 (Deterministic)
